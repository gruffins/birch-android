...
class EngineTest {
    ...
    fun `log() applies scrubbers`() {
        ...
        val metadataBlock = slot<() -> JSONObject>()

        every { source.toJson() } returns JSONObject()
        every { logger.log(any(), capture(metadataBlock), capture(block)) } returns true
        ...
        assert(metadataBlock.captured.invoke().optString("timestamp", "") != "")
    }
